...
set folderName "/home/meholmbe/optics-scan/results/$folderName-swing"
...
foreach group $groups {
        ...
        foreach index_rxterm [array names rxterm_setting_gty] { 
          ...
          foreach dfe [list 0] {
            foreach link $links {
              ...
              set scanName "$DFE RXTERM-$rxterm_setting_gty($index_rxterm) TXDIFF-$txdiff_setting_gty($index_diff) TXPOST-$txpost_setting_gty($index_post) TXPRE-$txpre_setting_gty($index_pre) Scan $groupName $linkName"
              ...
              if { $error_count < $best_err_errors($link) || ($error_count == $best_err_errors($link) && $open_area > $best_err_area($link))} {
                ...
                set best_err_cfg($link) "($txdiff_setting_gty($index_diff),$txpre_setting_gty($index_pre),$txpost_setting_gty($index_post),$rxterm_setting_gty($index_rxterm))"
                ...
                lappend best_err_cfg($link) "($txdiff_setting_gty($index_diff),$txpre_setting_gty($index_pre),$txpost_setting_gty($index_post),$rxterm_setting_gty($index_rxterm))"
              }
  ...
  foreach link $links {
    ...
    append text "\"txDiff,txPre,txPost,rxTerm\" : \[$best_err_cfg($link)\], \n"
    puts $fbest_err $text
