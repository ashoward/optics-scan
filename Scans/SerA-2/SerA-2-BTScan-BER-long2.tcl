...
# Output folder                                                                                                                           
set folderName "/home/luis/Projects/optics-scan/results/SerA-2_BTScan_BER_long2_$folderTime"
# Board name                                                                                                                              
set board "xcvu9p_0"
      ...
      foreach index_rxemp [array names rxemp_setting] {
        ...
        foreach index_diff [array names txdiff_setting_gty] {
              ...
              foreach index_rxterm [array names rxterm_setting_gty] { 
                ...
                foreach dfe [list 0] {
                  ...
                  foreach link $links {
                    ...
                    puts "RXAMP     : $rxamp_setting($index_rxamp)"
                    puts "RXPRE-EMP : $rxemp_setting($index_rxemp)"
		  ...
		  foreach link $links {
                    ...
                    set received_bits [ get_property RX_RECEIVED_BIT_COUNT [get_hw_sio_links $link] ] 
		    while {$received_bits < [ expr { pow(10, 15) } ] } {
			refresh_hw_device -update_hw_probes false [lindex [get_hw_devices $board] 0] 
			set received_bits [ get_property RX_RECEIVED_BIT_COUNT [get_hw_sio_links $link] ] 
		    }
		    set received_bits [ get_property RX_RECEIVED_BIT_COUNT [get_hw_sio_links $link] ] 
                    ...
                    append text "OpenA : $open_area($link)\n"
                    append text "DFE   : $DFE\n"
