...
set MGTtypeId [ dict create GTH "_1_0_0/IBERT" GTY "_1_0_0/IBERT" ]

# Set base directory of optics scan
set tclBase "/home/meholmbe/optics-scan"
# Open files
set ku15p_sm1_v2_fileName "$tclBase/json/Board03/ku15p_sm1_v2_connectivity.json"
set configFileName "$tclBase/json/Board03/connections_configTx1Rx1.json"
...
dict for { id c } $connections {
    ...
    if { $connectionType == "Cu12" } {
        ...
        for {set i [expr $nConnections-1]} {$i > -1} {incr i -1} {
	    set j [expr { $i - 1 }]
	    set testj [expr { $i>0? ($i+1) : ($i-1) }]
	    set choose [expr { $i % 2 ? "Odd" : "Even" }]
	    if { $choose == "Odd" } {
		puts $choose
		puts $j
		puts [dict get $socketBtmp $j]
		dict set socketB $j [dict get $socketBtmp $j] 
	    } elseif { $testj > -1 } {
		puts $choose
		puts $testj
		puts [dict get $socketBtmp $testj]
		dict set socketB $testj [dict get $socketBtmp $testj] 
	    } else {
		puts [dict get $socketBtmp [expr {$i+1}] ]
		dict set socketB [expr {$i+1}] [dict get $socketBtmp [expr {$i+1}]]
	    }
        }
